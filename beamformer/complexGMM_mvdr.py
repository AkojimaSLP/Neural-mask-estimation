...
class complexGMM_mvdr:
    ...
    def get_spatial_correlation_matrix_from_mask_for_LSTM(self, speech_data, speech_mask, noise_mask=np.array([None]), less_frame=10):      
        """
        if noise_mask.any() == None:
            print('make_noise_mask')
            noise_mask = (1 - speech_mask)+0.01
    ...
    def get_mvdr_beamformer(self, R_x, R_n):
        ...
        steering_vector_save = np.ones((number_of_channels, number_of_bins), dtype=np.complex64)
        for f in range(0, number_of_bins):
            _, eigen_vector = np.linalg.eig(R_x[:, :, f])
            steering_vector = eigen_vector[:, 0]
            steering_vector_save[:, f] = eigen_vector[:, 0]
            ...
            beamformer[:, f] = w
        return (beamformer, steering_vector_save)
    
    def get_mvdr_beamformer_onehot(self, R_x, R_n, onehot):
        number_of_channels, _, number_of_bins = np.shape(R_x)
        beamformer = np.ones((number_of_channels, number_of_bins), dtype=np.complex64)        
        for f in range(0, number_of_bins):
            #_, eigen_vector = np.linalg.eig(R_x[:, :, f])
            #steering_vector = eigen_vector[:, 0]
            Rn_inv = np.linalg.pinv(R_n[:, :, f], rcond=self.beamformer_inv_threshold)
            w1 = np.matmul(Rn_inv, R_x[:, :, f])
            w1 = np.matmul(w1, onehot)
            w2 = np.trace(np.matmul(Rn_inv, R_x[:, :, f]))
            w2 = np.reshape(w2, [1, 1])
            w = w1 / w2
            w = np.reshape(w, number_of_channels)
            beamformer[:, f] = w
        return (beamformer, np.ones(10))
        
    def get_mvdr_beamformer_onehot2(self, R_x, R_n,onehot):
        number_of_channels, _, number_of_bins = np.shape(R_x)
        beamformer = np.ones((number_of_channels, number_of_bins), dtype=np.complex64)        
        for f in range(0, number_of_bins):
            #_, eigen_vector = np.linalg.eig(R_x[:, :, f])
            #steering_vector = eigen_vector[:, 0]
            steering_vector = onehot
            Rn_inv = np.linalg.pinv(R_n[:, :, f], rcond=self.beamformer_inv_threshold)
            w1 = np.matmul(Rn_inv, steering_vector)
            w2 = np.matmul(np.conjugate(steering_vector).T, Rn_inv)
            w2 = np.matmul(w2, steering_vector)
    ...
    def get_mvdr_beamformer_without_onehot(self, R_x, R_n):
        number_of_channels, _, number_of_bins = np.shape(R_x)
        beamformer = np.ones((number_of_channels, number_of_bins), dtype=np.complex64)        
        for f in range(0, number_of_bins):
            #_, eigen_vector = np.linalg.eig(R_x[:, :, f])
            ...
            Rn_inv = np.linalg.pinv(R_n[:, :, f], rcond=self.beamformer_inv_threshold)
            w1 = np.matmul(Rn_inv, R_x[:, :, f])
            w1 = np.matmul(w1, np.ones(number_of_channels) / number_of_channels)
            #w2 = np.matmul(np.conjugate(steering_vector).T, Rn_inv)
            w2 = np.trace(np.matmul(Rn_inv, R_x[:, :, f]))
            w2 = np.reshape(w2, [1, 1])
            w = w1 / w2
